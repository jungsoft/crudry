defmodule HandleChangesetErrorsTest do
  ...
  test "translate can't be blank changeset error" do
    ...
    assert HandleChangesetErrors.call(resolution, :_) == build_resolution("username can't be blank")
  end

  test "translate length changeset error" do
    changeset = User.changeset(%User{}, %{username: "a"})
    resolution = build_resolution(changeset)
    assert HandleChangesetErrors.call(resolution, :_) == build_resolution("username should be at least 2 character(s)")
  end

  test "translate multiple changeset errors" do
    changeset = Post.changeset(%Post{}, %{})
    resolution = build_resolution(changeset)
    assert HandleChangesetErrors.call(resolution, :_) == build_resolution(["title can't be blank", "user_id can't be blank"])
  end
  ...
  test "translate nested changeset errors" do
    ...
    resolution = build_resolution(changeset)
    assert HandleChangesetErrors.call(resolution, :_) == build_resolution(["posts: title can't be blank", "posts: user_id can't be blank", "username can't be blank"])
  end
  ...
  test "translate deeply nested changeset errors" do
    ...
    resolution = build_resolution(changeset)
    assert HandleChangesetErrors.call(resolution, :_) == build_resolution(["likes: post_id can't be blank", "likes: user_id can't be blank", "posts: title can't be blank", "posts: user_id can't be blank", "username can't be blank"])
  end
